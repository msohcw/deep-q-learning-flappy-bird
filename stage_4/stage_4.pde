...
void setup(){
  ...
  agent = new Learner(0.000001f, 0.000003f, 0.95);
}
...
void record(int points){
  ...
  if(game.episodes % HISTORY_INTERVAL == 0){ // sorting is time consuming
...
float significant(double v, int d){
  return ( round((float) v * pow(10,d)) / pow(10,d) );
}
