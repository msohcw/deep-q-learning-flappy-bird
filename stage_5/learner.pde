...
class Learner{
 ...
 //              	return 0;
	...
	void experienceReplay(){
		...
		double[] experienceErrors = new double[replayLength];
		...
		for(int i = 0; i < replayLength; i++){
			...
			experienceErrors[i] = abs((float)(targetMatrix[i][e.action] - outputMatrix[i][e.action]));
